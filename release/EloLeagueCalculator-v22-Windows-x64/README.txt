...
- The Simulator button runs read-only Monte Carlo single round-robin seasons
  ...
  tied on match wins are separated by head-to-head match score and then
  head-to-head game percentage before the remaining season tiebreakers. Players
  tied on every tiebreaker share title and rank credit. An optional random seed
...
- Standings include match W-D-L, match score percentage, individual game W-L,
  ...
  Rankings use Elo, head-to-head match score percentage among Elo-tied players,
  head-to-head game percentage, overall match score percentage, SB, overall
  game win percentage, and then natural player-name order, so default player
  numbers display numerically.

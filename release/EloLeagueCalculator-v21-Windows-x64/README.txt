...
- The Simulator button runs read-only Monte Carlo single round-robin seasons
  ...
  tied on match wins are separated by head-to-head score before the remaining
  season tiebreakers, while players tied on every tiebreaker share title and
  rank credit. An optional random seed makes results reproducible.
...
- Standings include match W-D-L, match score percentage, individual game W-L,
  ...
  overall match score percentage, SB, game win percentage, and then natural
  player-name order, so default player numbers display numerically.
